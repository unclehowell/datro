...
| `OLLAMA_DOWN`   | The chat route could not reach ollama (`ECONNREFUSED` / `fetch failed`). The local-first path returned null and the cloud fallback was also empty. | Start ollama: `ollama serve`. On Termux the model auto-loads on first call — allow up to 60 s.                               |
...
| `OLLAMA_DOWN`     | `think`    | LLM stack refused to come up (ECONNREFUSED on `http://localhost:11434` or omniroute `:20128`).           | Start ollama: `ollama serve`. If omniroute is missing, run the installer's omniroute step.                  |
| `PIPELINE_FAIL`   | any stage  | An unexpected exception escaped the voicemail pipeline (the `catch` in `processVoicemailAsync`).         | Re-record the voicemail. If it repeats, capture the full job JSON from `~/.fcukproxy/voicemail/jobs/<id>.json`. |
| `PIPELINE_RESTART`| any stage  | The GUI was restarted while this voicemail was being processed; the in-memory job state was lost but the persisted job file was marked stale. | Re-record the voicemail. (The recorded audio is still on disk; we just can't continue an interrupted LLM call.) |
