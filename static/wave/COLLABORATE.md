...
6. Restrict git to only get the directory/subdirectory you need. Here you have two choices:
 ...
 - Option A: Simple/ automated configuration method:
      ...
      wget https://raw.githubusercontent.com/unclehowell/datro/master/checkout.sh && chmod +x checkout.sh
...
1. Create and execute a script to install the subrepo command:  
      ...
      ` svn co --depth files https://github.com/unclehowell/datro/branches/gh-pages/static/splashpage/img/ `

      
      ### Find and relace in all files with the extensin .html

      find ./ \( -type d -name .html -prune \) -o -type f -print0 | xargs -0 sed -i 's/G-6BTNPTMXBF/G-0R4RHPQ953/g'
