...
// Find all latest/ dirs — marker is index.html (generated by process-intray.js)
  ...
  try {
    for (const entry of readdirSync(dir)) {
      ...
      try {
        ...
        if (entry === 'latest' && existsSync(join(full, 'index.html'))) {
          results.push(full);
        } else {
...
{
  const SKIP = new Set(['.', '..', 'node_modules', 'functions', 'wiki', 'raw', '_intray', '_outtray', '_intray_backup', '_test', '_theme-docs', '_theme-explorer', '_theme-vitepress', '_vitepress-theme']);
  const catEntries = [];
...
// Step 2: track hashes (no build step — index.html generated by process-intray.js)
const latestDirs = findLatestDirs(ROOT);
console.log(`Found ${latestDirs.length} docs`);
