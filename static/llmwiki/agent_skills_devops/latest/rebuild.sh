#!/usr/bin/env bash
# rebuild.sh for agent_skills_devops — matches library standardisation
# Builds MD sources via Sphinx+myst-parser → HTML (RTD theme + blue.sh) + PDF

set -e
cd "$(dirname "$0")"

echo "Building Agent Skills Devops..."
...
<html><body></body>
<script>window.open("./agent-skills-devops.pdf","_self");</script>
<script language="JavaScript">setTimeout("window.history.go(-1)",500);</script>
</html>
PEOF

echo "Done: agent_skills_devops"
