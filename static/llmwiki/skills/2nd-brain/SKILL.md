...
## What This Is
`~/brain/` (resolves to `/home/ubuntu/datro/static/llmwiki/` on this server) is the shared intelligence between all agents on this machine.
Everything we learn flows through here.

**Important:** The brain path may vary by user profile. Always use `~/brain/` or `${BRAIN_ROOT}` rather than hardcoding `/home/ubuntu/datro/static/llmwiki/`.
