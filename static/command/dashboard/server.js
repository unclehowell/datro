...
const LOGIN_PASSPHRASE = process.env.LOGIN_PASSPHRASE || 'Burgerking';
const TOKEN_SECRET = process.env.TOKEN_SECRET || crypto.randomBytes(32).toString('hex');
const validTokens = new Set();

function generateToken() {
  const payload = JSON.stringify({ t: Date.now() + 86400000, s: TOKEN_SECRET.slice(0, 8) });
  const hmac = crypto.createHmac('sha256', TOKEN_SECRET).update(payload).digest('hex').slice(0, 12);
...
function validateToken(token) {
  ...
  try {
    ...
    if (parsed.t < Date.now()) { validTokens.delete(token); return false; }
    const expectedHmac = crypto.createHmac('sha256', TOKEN_SECRET).update(payload).digest('hex').slice(0, 12);
    return expectedHmac === parts[1];
  } catch { validTokens.delete(token); return false; }
}

function authMiddleware(req, res, next) {
  const publicPaths = ['/api/login', '/api/status', '/api/version'];
  if (publicPaths.includes(req.path)) return next();
  if (req.method === 'GET' && req.path === '/') return next();
  const auth = req.headers.authorization || '';
  const token = auth.startsWith('Bearer ') ? auth.slice(7) : (req.query.token || req.cookies?.token);
  if (validateToken(token)) return next();
  if (req.path.startsWith('/api/')) {
    return res.status(401).json({ error: 'Unauthorized. POST /api/login with passphrase.' });
  }
  next();
}
...
// Push all pending changes to all branches
app.post('/api/push', async (req, res) => {
  try {
    const repoPath = REPO_PATH;
    const msg = `cnei webgui: bulk push ${new Date().toISOString().slice(0, 16)}`;
    const { execSync } = require('child_process');
    let output = '';
    output += execSync(`git add -A`, { cwd: repoPath, timeout: 15000 }).toString();
    output += execSync(`git commit -m "${msg}" --no-verify`, { cwd: repoPath, timeout: 15000 }).toString();
    const branches = (await fsPromises.readdir(MASTERS_DIR)).filter(f => f.endsWith('.md')).map(f => f.replace('.md', ''));
    for (const b of branches) {
      try {
        output += execSync(`git push origin "${b}"`, { cwd: repoPath, timeout: 60000 }).toString();
      } catch (e) {
        output += `[${b}] push failed: ${e.message}\n`;
      }
    }
...
// Pull latest md files from all branches
app.post('/api/pull', async (req, res) => {
  try {
    ...
    for (const b of branches) {
      try {
        const fetchOut = execSync(`git fetch --depth=1 origin "${b}"`, { cwd: repoPath, timeout: 30000 }).toString();
        const mergeOut = execSync(`git merge FETCH_HEAD --no-edit --allow-unrelated-histories`, { cwd: repoPath, timeout: 30000 }).toString();
        if (fetchOut || mergeOut) output += `[${b}] ${fetchOut}${mergeOut}`;
      } catch (e) {
        output += `[${b}] pull failed: ${e.message}\n`;
      }
    }
    res.json({ output: output || 'All up to date.', success: true });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
});

const APP_VERSION = 'command-v1.0.0.00';
