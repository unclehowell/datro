...
export async function onRequestPost(context: any) {
  try {
    ...
    // ── Use signature generated by the client ─────────────────────
    // ClaimForm.tsx already computes btoa(JSON.stringify(signatureData))
    // using the exact values the user submitted. Using it here avoids
    // any mismatch from server-side reformatting.
