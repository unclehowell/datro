...
if echo "$TASK_LOWER" | grep -q "theme\|color\|style\|css\|toggle"; then
  ...
  if command -v opencode >/dev/null 2>&1; then
    ...
    # Manual theme addition with color palette support
    mkdir -p src/components 2>/dev/null || true
    
    if echo "$TASK_LOWER" | grep -q "color\|palette\|blue\|green\|purple"; then
      cat > src/components/ThemeToggle.tsx << 'COLOREOF'
...
COLOREOF
      log "Updated ThemeToggle.tsx with color palette"
    else
